...
{ // checking whether num is present in col or not
    for (int i = 0; i < X; i++)
    {
...
{ // checking whether num is present in row or not
    for (int i = 0; i < X; i++)
    {
        if (sudoku_problem[row][i] == num)
        { // to check if there is already a entry equal to val in row
            return false;
        }
    }
    return true;
...
void sudokuGrid()
{ // To print the sudoku sudoku_problem after solve
...
{ // To know empty location and update row and column
    for (row = 0; row < X; row++)
        for (col = 0; col < X; col++)
...
bool solveSudoku()
{
    ...
    { // For valid numbers only 1 - 9
        if (ValidPlace(row, col, num))
        { // checking validation, if yes, put the number in the grid
